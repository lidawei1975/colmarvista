 ...
 * 3. Draw_spectrum will push the new spectrum object to all_spectra array. Order in the array depends on who is generated first (ft2 may jump before fid even if fid is read first)
    ...
    document.getElementById('fid_file_form').addEventListener('submit', function (e) {
        ...
        function process_fid_files(processing_flag, spectrum_index) {
            ...
            */
            ...
            {
                fid_process_parameters.pseudo3d_children = all_spectra[spectrum_index].pseudo3d_children;
            }
            else
            {
        ...
        {
            current_fid_files = all_spectra[current_reprocess_spectrum_index].fid_process_parameters.file_data;
...
my_contour_worker.onmessage = (e) => {
    ...
    else if (e.data.spectrum_type === "full" && e.data.spectrum_index === main_plot.levels_length_negative.length)
    {
        ...
        {   
            ...
            main_plot.colors.push(hexToRgb(all_spectra[e.data.spectrum_index].spectrum_color));
            ...
            else
            {
                 ...
                 */
                let highest_level = all_spectra[e.data.spectrum_index].levels.length - 1;
            ...
            main_plot.spectral_information.push({
                n_direct: all_spectra[e.data.spectrum_index].n_direct,
                n_indirect: all_spectra[e.data.spectrum_index].n_indirect,
                x_ppm_start: all_spectra[e.data.spectrum_index].x_ppm_start,
                x_ppm_step: all_spectra[e.data.spectrum_index].x_ppm_step,
                y_ppm_start: all_spectra[e.data.spectrum_index].y_ppm_start,
                y_ppm_step: all_spectra[e.data.spectrum_index].y_ppm_step,
                x_ppm_ref: all_spectra[e.data.spectrum_index].x_ppm_ref,
                y_ppm_ref: all_spectra[e.data.spectrum_index].y_ppm_ref,
            });
        ...
        else if(e.data.contour_sign === 1)
        {
            ...
            main_plot.colors_negative.push(hexToRgb(all_spectra[e.data.spectrum_index].spectrum_color_negative));

            if(total_number_of_experimental_spectra <= 4){
                main_plot.contour_lbs_negative.push(0);
            }
            else{
                let highest_level = all_spectra[e.data.spectrum_index].negative_levels.length - 1;
    ...
    /**
     * Type is full and  all_spectra.length === main_plot.levels_length.length
...
function reduce_contour(index,flag) {
    ...
    */
    ...
    my_contour_worker.postMessage({ response_value: all_spectra[index].raw_data, spectrum: spectrum_information });
...
function draw_spectrum(result_spectra, b_from_fid,b_reprocess,pseudo3d_children=[])
{
    ...
    for(let i=0;i<result_spectra.length;i++)
    {
        ...
        let spectrum_information = {
             ...
             * @var spectrum_index: index of the spectrum in the all_spectra array
 ...
 * Similar to draw_spectrum() but no need to update all_spectra array (we loaded it)
 */
function draw_spectrum_from_loading()
{
    ...
    for(let i=0;i<all_spectra.length;i++)
    {
        ...
        let spectrum_information = {
             ...
             * n_direct,n_indirect, and levels are required for contour calculation
             */
            n_direct: all_spectra[i].n_direct,
            n_indirect: all_spectra[i].n_indirect,
            levels: all_spectra[i].levels,
             ...
             */
            spectrum_type: "full",
            spectrum_index: all_spectra[i].spectrum_index,
            spectrum_origin: all_spectra[i].spectrum_origin,
            contour_sign: 0
        };
        my_contour_worker.postMessage({ response_value: all_spectra[i].raw_data, spectrum: spectrum_information });

        /**
         * Negative contour calculation for the spectrum
         */
        spectrum_information.contour_sign = 1;
        spectrum_information.levels = all_spectra[i].negative_levels;
        my_contour_worker.postMessage({ response_value: all_spectra[i].raw_data, spectrum: spectrum_information });
    }
}
...
function remove_spectrum(index)
{
     ...
     */
    all_spectra[index].raw_data = new Float32Array();
    all_spectra[index].header = new Float32Array();
    all_spectra[index].levels = [];
    all_spectra[index].negative_levels = [];
    /**
     * Remove this spectrum from its original spectrum's reconstructed_indices
     */
    if(all_spectra[index].spectrum_origin >= 0)
    {
        let origin_index = all_spectra[index].spectrum_origin;
        all_spectra[origin_index].reconstructed_indices.splice(all_spectra[origin_index].reconstructed_indices.indexOf(index), 1);
    }

    all_spectra[index].spectrum_origin = -3; // -3 means the spectrum is removed
    all_spectra[index].visible = false; // set visible to false
    all_spectra[index].picked_peaks_object = null;
    all_spectra[index].fitted_peaks_object = null;
     ...
     */
    if(index === all_spectra.length - 1) //last spectrum
    {
...
function apply_current_pc_or_auto_pc(flag)
{
    ...
    if(flag==0)
    {
        ...
        if(main_plot.current_spectral_index === current_reprocess_spectrum_index)
        {
            ...
            all_spectra[current_reprocess_spectrum_index].fid_process_parameters.phase_correction_direct_p0 = v;
            ...
            all_spectra[current_reprocess_spectrum_index].fid_process_parameters.phase_correction_direct_p1 = v;
            ...
            all_spectra[current_reprocess_spectrum_index].fid_process_parameters.phase_correction_indirect_p0 = v;
            ...
            all_spectra[current_reprocess_spectrum_index].fid_process_parameters.phase_correction_indirect_p1 = v;
            /**
             * To be safe, uncheck auto phase correction
             */
            document.getElementById("auto_direct").checked = false;
            document.getElementById("auto_indirect").checked = false;
            all_spectra[current_reprocess_spectrum_index].fid_process_parameters.auto_direct = false;
            all_spectra[current_reprocess_spectrum_index].fid_process_parameters.auto_indirect = false;
        }
    ...
    let n_size = all_spectra[index].n_direct * all_spectra[index].n_indirect * 4;
    ...
    webassembly_worker.postMessage({
        ...
        spectrum_name: all_spectra[main_plot.current_spectral_index].filename,
    });
    /**
     * Let user know the processing is started
     */
