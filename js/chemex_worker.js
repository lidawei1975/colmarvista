...
async function initWorker() {
  try {
    ...
    self.postMessage({ type: "status", text: "Installing ChemEx dependencies (pydantic, rich, etc.)..." });
    try {
      await micropip.install(["pydantic", "rich", "annotated-types", "cachetools", "emcee"]);
    } catch (e) {
    ...
    const wheelUrl = getUrl("../chemex/chemex-2026.9.1-py3-none-any.whl");
    ...
    pyodide.FS.writeFile("/home/pyodide/chemex-2026.9.1-py3-none-any.whl", wheelBytes);
...
wheel_file = os.path.join(home_dir, "chemex-2026.9.1-py3-none-any.whl")
...
async function writeCestDataset() {
  const manifestUrl = getUrl("../chemex/CEST_15N/files_manifest.json?t=" + Date.now());
